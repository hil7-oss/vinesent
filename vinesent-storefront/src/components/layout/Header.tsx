...
export default function Header() {
  ...
  useEffect(() => {
    fetch('/api/fastapi/promo-banners', { cache: 'no-store' })
      .then(r => r.ok ? r.json() : [])
      .then((list: any[]) => {
        const active = Array.isArray(list) ? list.filter(b => b && b.active) : []
  ...
  useEffect(() => {
    if (!promoBanners.some(b => b.showTimer)) return
      ...
      <div className="sticky top-0 left-0 right-0 z-50">
        {promoBanners.length > 0 && (
          <section
            ...
            style={{ backgroundColor: promoBanners[0].bgColor || '#000', color: promoBanners[0].textColor || '#fff', borderColor: 'rgba(255,255,255,0.12)' }}
          >
            <div className="max-w-full mx-auto px-4 lg:px-6 py-1.5 lg:py-2.5 flex flex-col sm:flex-row items-center justify-center gap-2 overflow-hidden">
              <div className="relative w-full flex items-center justify-center overflow-hidden">
                <div className={`flex items-center gap-12 ${promoBanners.some(b => b.showAnimation) ? 'animate-marquee' : ''}`}>
                  {promoBanners.map((b, i) => (
                    <div key={b.id || i} className="flex items-center gap-6 shrink-0">
                      <div className="flex items-center gap-3">
                        ...
                        {b.buttonText && (
                          <Link
                            ...
                            style={{ backgroundColor: b.buttonBgColor || '#fff', color: b.buttonTextColor || '#000' }}
                          >
                            {b.buttonText}
                          </Link>
                        )}
                      ...
                      {b.showTimer && (
                        <div className="flex items-center gap-2 text-[11px] sm:text-[12px] shrink-0" aria-label="countdown">
                          {(() => {
                            const endsAt = b.timerEndsAt ? new Date(b.timerEndsAt).getTime() : 0
                            ...
                            return (
                              <>
                                <span style={{ color: b.textColor ? `${b.textColor}99` : 'rgba(255,255,255,0.6)' }}><b style={{ color: b.textColor || '#fff', fontSize: '13px', fontWeight: 600 }}>{pad2(d)}</b> дн</span>
                                <span style={{ color: b.textColor ? `${b.textColor}99` : 'rgba(255,255,255,0.6)' }}><b style={{ color: b.textColor || '#fff', fontSize: '13px', fontWeight: 600 }}>{pad2(h)}</b> год</span>
                                <span style={{ color: b.textColor ? `${b.textColor}99` : 'rgba(255,255,255,0.6)' }}><b style={{ color: b.textColor || '#fff', fontSize: '13px', fontWeight: 600 }}>{pad2(m)}</b> хв</span>
                                <span style={{ color: b.textColor ? `${b.textColor}99` : 'rgba(255,255,255,0.6)' }}><b style={{ color: b.textColor || '#fff', fontSize: '13px', fontWeight: 600 }}>{pad2(s)}</b> сек</span>
                              </>
                            )
                          })()}
                        </div>
                  ...
                  {/* Duplicated for marquee loop if animation is active */}
                  {promoBanners.some(b => b.showAnimation) && promoBanners.map((b, i) => (
                    <div key={`dup-${b.id || i}`} className="flex items-center gap-6 shrink-0">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] sm:text-[11px] tracking-[0.12em] uppercase font-bold whitespace-nowrap">
                          {b.text}
                        </span>
