...
from ..liqpay import LiqPayHelper
...
def _liqpay():
    ...
    prv = os.environ.get("LIQPAY_PRIVATE_KEY")
    if APP_ENV == "production" and (not pub or not prv):
    ...
    if not prv:
        prv = "sandbox_prv"
    return LiqPayHelper(pub, prv)


def _liqpay_callback_url():
...
def liqpay_pay_order(orderId: str, db: Session = Depends(get_db)):
    ...
    params = {
        ...
        "currency": os.environ.get("LIQPAY_CURRENCY", "UAH"),
        "description": f"Order {orderId}",
        "order_id": orderId,
        "sandbox": 1 if (os.environ.get("APP_ENV", "").lower() != "production") else 0,
        "server_url": _liqpay_callback_url(),
        "result_url": f"{_liqpay_result_base().rstrip('/')}/checkout/{orderId}/result",
    }
