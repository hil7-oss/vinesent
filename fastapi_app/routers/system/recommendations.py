...
from fastapi_app.models import Product
from fastapi_app.schemas import ProductOut, RecommendationRes

router = APIRouter(prefix="/api/v1/products", tags=["recommendations"])

def _get_products_out(product_ids: list[str], db: Session) -> list[ProductOut]:
    if not product_ids:
        return []
    ...
    by_id = {str(product.id): ProductOut.model_validate(product, from_attributes=True) for product in products}
...
def get_product_recommendations(productId: str, db: Session = Depends(get_db)):
    ...
    def fetch_products(ids):
        return _get_products_out([pid for pid in ids if pid != productId], db)
