            ...
            <div>
              ...
              <div className="space-y-4">
                {(form.deliveryMethod === 'nova_branch' || form.deliveryMethod === 'nova_courier') && (
                  <div>
                    <label className="block text-[12px] font-medium uppercase tracking-wide mb-1.5">Місто</label>
                    {form.deliveryMethod === 'nova_branch' && NP_API_KEY ? (
                      <div ref={npCityRootRef} className="np-select-root" />
                    ) : (
                      <input value={form.city} onChange={e => updateField('city', e.target.value)} className="w-full h-12 px-4 rounded-xl border border-black/15 text-[14px] outline-none focus:border-black/40 transition" placeholder="Введіть місто" />
                    )}
                  </div>
                )}
                {form.deliveryMethod === 'nova_branch' && (
                  <div>
                    <label className="block text-[12px] font-medium uppercase tracking-wide mb-1.5">Відділення</label>
                    {NP_API_KEY ? (
                      <div ref={npWarehouseRootRef} className="np-select-root" />
                    ) : (
                      <input value={form.branch} onChange={e => updateField('branch', e.target.value)} className="w-full h-12 px-4 rounded-xl border border-black/15 text-[14px] outline-none focus:border-black/40 transition" placeholder="Виберіть відділення" />
                    )}
                  </div>
                )}
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={() => setStep('data')} className="flex-1 py-4 rounded-2xl border border-black/15 text-[14px] font-semibold uppercase hover:bg-gray-50 transition">Назад</button>
                <button onClick={() => setStep('payment')} className="flex-1 bg-[#111] text-white py-4 rounded-2xl text-[14px] font-semibold uppercase hover:bg-black/80 transition">Далі</button>
              </div>
            </div>
          )}
