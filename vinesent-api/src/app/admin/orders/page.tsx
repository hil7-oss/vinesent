        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {(order.customerName || order.customerPhone || order.customerEmail || order.customerAddress) && (
            <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
              <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Покупець</div>
              {order.customerName && (
              ...
              {order.customerAddress && (
                <div className="flex items-start gap-2">
                  <svg className="w-4 h-4 text-gray-300 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                  <span className="text-[12px] text-gray-600">{order.customerAddress}</span>
                </div>
              )}
            </div>
          )}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-[13px] font-medium text-gray-800 truncate">
                          {it.slug
                            ? <Link href={`/product/${it.slug}`} className="hover:underline">{it.name || it.productId.slice(0, 8)}</Link>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {order.customerName && <><span className="text-gray-200">·</span><span className="text-[12px] text-gray-600 font-medium">{order.customerName}</span></>}
          </div>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50/50 transition group">
                    ...
                    {/* customer */}
                    <td className="px-4 py-3">
                      <div className="text-[13px] font-semibold text-gray-800">{o.customerName || '—'}</div>
                      {o.customerPhone && <div className="text-[11px] text-gray-400">{o.customerPhone}</div>}
                      {o.customerAddress && <div className="text-[10px] text-gray-400 truncate max-w-[160px]">{o.customerAddress}</div>}
                    </td>
                    {/* date */}
