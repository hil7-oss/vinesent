...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/api/v1/products`).then(r => r.ok ? r.json() : []),
      fetch(`${API_BASE}/api/v1/categories`).then(r => r.ok ? r.json() : []),
      fetch(`${API_BASE}/api/v1/orders`).then(r => r.ok ? r.json() : []),
      fetch(`${API_BASE}/api/v1/content`).then(r => r.ok ? r.json() : null),
  ...
  const saveHeroSlides = async () => {
    ...
    await fetch(`${API_BASE}/api/v1/content/hero-slides`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(heroSlides.map(s => ({ ...s, position: 'hero' }))) })
  ...
  const saveCollection = async (col: AdminCollection) => {
    setSavingContent(true)
    await fetch(`${API_BASE}/api/v1/content/collections/${col.key}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: col.title, description: col.description, productIds: col.productIds.split(',').map(v => v.trim()).filter(Boolean) }) })
    setSavingContent(false)
  }
  const addCollection = async () => {
    const key = newCollectionKey.trim().toUpperCase(); if (!key) return
    setSavingContent(true)
    const res = await fetch(`${API_BASE}/api/v1/content/collections`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key, title: key, description: '', productIds: [] }) })
  ...
  const removeCollection = async (key: string) => {
    setSavingContent(true)
    await fetch(`${API_BASE}/api/v1/content/collections/${key}`, { method: 'DELETE' })
